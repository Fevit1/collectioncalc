...
// The legacy /api/messages comprehensive-grade path was REMOVED in Batch 4C: it
// returned the model's raw (un-snapped, possibly off-scale) grade and duplicated
// grade logic. app.html overrides window.generateGradeReport with a version that
// posts to /api/grade, which grades AND snaps to the canonical CGC scale
// server-side (single source of truth: grading_engine). This stub keeps the name
// defined for the step-skip caller above; it is never executed in app.html (the
// override shadows it). If grading.js is ever used standalone, wire this to
// /api/grade — do NOT re-add an /api/messages grader.
async function generateGradeReport() {
    console.warn('generateGradeReport: legacy /api/messages grade path removed in Batch 4C — use the /api/grade flow (app.html override).');
}
