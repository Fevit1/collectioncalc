...
**State at close:** `git log -1` = `9318e8d` + the close-out records commit Mike runs after this write (that commit is the true HEAD; `git log -1` shows it) · 0 ahead of origin · working tree CLEAN after that commit (untracked stray files at the root and in docs/ predate 09-17 and are Mike's) · last deploy `d21fdc9`
...
`ebay-collector` 1.5.0 · `whatnot-valuator` 2.47.0 · `slab-guard-monitor` 1.0.1. Claude-initiated API spend for the week: 09-19 $6.62 · 09-20 $4.12 · 09-21 $6.50 · 09-22 $6.37 (all measured; `docs/API_SPEND_LEDGER.md`).
...
- **The pricing decision** (Mike): per-use credit, or a small Pro cap priced against **$0.15 one pass / $0.26 three
  passes** — both measured; one pass matched three on 42 / 42 (`docs/technical/SIGNATURE_IMAGE_AND_PASSES_2026-09-22.md`).
- **The image lever is the product change that matters:** crop to the signature (or ask for a close photo) before
  matching; treat a target under ~150 px long edge as "can't tell". Full-res crops name 83% of signers, thumbnails 69%
  with the programme's first wrong name at 0.88.
- Label-reading leak (prompt or crop); Design B (6 of 11 pool misses had the true name volunteered); `title_year`
  backfill for signed rows; the review-queue SQL views mixing v1 shares and v2 scores; the route still accepting
  arbitrary `signature_data` from the client (escaped on display, not validated on write).
- Item 20's marketplace and Whatnot halves; item 21's follow-on (dashboard sums, My Val sort, the "Profit on Rated
  Comics" headline); item 23 (operator key → `is_internal`); item 24 (pass progress); `terms.html` "Last Updated".
- The detection post-mortem for the 06-23 → 09-18 signature outage; the canary-call lesson for
  `LESSONS_CROSS_PROJECT.md`; the L-SW-2026-031 candidate.
- The Slab Guard measured run toward the bar (allow-list 3,25,26 is set for it); the white paper and licensing
  proposal to carry the bar; the twelve content pages (Bilbo drafts); the "before you slab" grading-prompt change.
- (3)'s cost sample says a Design A pool averages ~23k tokens; the harness figure of 38.9k was a random pool.

## 2026-09-22 — 🌙 **CLOSE-OUT. Tuesday's two runs done, $6.37 measured: (1) the IMAGE LEVER — full-resolution crops name 83% of signers with 0 wrong, 80-px thumbnails name 69% with 1 wrong; (2) ONE PASS vs THREE — identical top-1 and identical floor decision on 42 of 42. Report: `docs/technical/SIGNATURE_IMAGE_AND_PASSES_2026-09-22.md`. Nothing deployed today; nothing deploys tomorrow. The offline note is above.**

**MOST RECENT CHANGE (Rule 5): one pass equals three on this set (42/42), measured 2026-09-22 — the pricing decision
now has both numbers ($0.15 one pass, $0.26 three). Supersedes "waits on Tuesday's one-pass number". The offline
note moved from `OFFLINE_NOTE_DRAFT_2026-09-23.md` (deleted) to the top of this file. `git log -1` = `9318e8d` before
the close-out commit; that commit is the last before the window.**
- Arm 1, 84 calls, $3.14 (est. $3.05). Arm 2, 84 calls, $3.23 (est. $2.80; longer outputs). Both stopped nowhere.
- **Recommendation left for Mike:** ship one pass with the breakpoint off (~$0.15) and re-run three only inside a band
  around the floor; crop to the signature before matching; treat a thumbnail-size target as "can't tell".
- **Render rollback wording in the note: NOT CONFIRMED** — Mike's message carried the placeholder "[confirmed as
  written / actual label is ...]". The lever is written from my knowledge of the Render dashboard (Deploys → Rollback
  to this deploy); Mike checks the label on his first return.
- Working tree after this write: the records files, the two result files, the harness, the report — all in the
  close-out commit line below. Nothing else.
