...
- **🆕 FOLLOW-UP UNIT DRAFTED IN TREE (2026-07-11 evening, Mike's ask, awaiting review — Phase-3-style one small unit):** (i) **admin.html resource status chip** — always-visible line under the Dependency Alert banner (Memory / DB / pool; amber ≥warn or streak >0; field names verified against the live prod payload); (ii) **public `/health` MINIMIZED to `{status, version}`** — it was exposing installed Stripe version, versions-behind count, and internal monitoring notes publicly; `check_all()` STILL runs inside it (no cron exists — health polling is the monitor's scheduler; the state-change email fires inside check_all), output+runtime flags moved behind `/api/admin/dependency-status`. Offline-verified 6/6 (minimal body, check_all invoked, exception → still 200 + never leaks, `/` same shape). Files: routes/utils.py, routes/admin_routes.py, admin.html. ⚠️ Post-ship note for humans: `curl /health` no longer shows dependency warnings — that detail is admin-only now.
- **Git truth (updated 2026-07-11 evening):** HEAD = `bf92fcc` (2(f)); beneath it `3ce6e2d` (docs) / `820b0ae` (Phase 4) / `901a49e` (Phase 3). Dirty (this arc) = the follow-up unit (routes/utils.py, routes/admin_routes.py, admin.html) + docs (this file + `docs/LAUNCH_READINESS.md`). Pre-existing dirt (NOT this arc, do not bundle): `.claude/worktrees/*` deletions/mods, `scripts/slabguard_crosscamera_test.py`, `tests/SlabGuardTests/TP_RESHOOT_PROTOCOL.md`.
