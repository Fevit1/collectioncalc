# Where We Left Off - Jul 12, 2026

## Session 117 (Jul 12, 2026) — Section F OPENED: checklist drafted + HEIC gate found (iPhone half blocked); item 2(d) /health DB check pulled forward, DRAFTED IN TREE awaiting ship

**MOST RECENT CHANGE (2026-07-12 PM, Rule 5): Mike's three calls landed — (1) HEIC = FIX (not waive; "really an orientation-normalization fix with HEIC as a side effect") → BUILT IN TREE same day, offline 17/17; (2) 2(d) APPROVED TO SHIP (Mike runs the block, sets dashboard healthCheckPath himself post-deploy); (3) load target APPROVED + F-L4 ceiling burst added (16–18 concurrent vs the 16 thread slots — find the ceiling, not the floor); checklist canonical in repo, Mike keeps a supplementary personal phone run-sheet (no merge). Two ship units now in tree awaiting Mike: 2(d) and the HEIC/orientation unit.**
...
- **Git truth (2026-07-12 PM):** HEAD = `7ee58ad` (clean before this session). Dirty (this arc): **2(d) unit** = `routes/utils.py`; **HEIC unit** = `requirements.txt`, `comic_extraction.py`, `routes/grading.py`, `routes/admin_routes.py`, `docs/technical/ARCHITECTURE.txt`; **docs** = `docs/SECTION_F_CHECKLIST.md` (new), `docs/LAUNCH_READINESS.md`, this file. Pre-existing dirt (NOT this arc, do not bundle): `.claude/worktrees/*`, `scripts/slabguard_crosscamera_test.py`, `tests/SlabGuardTests/TP_RESHOOT_PROTOCOL.md`.
- **Ship block (Mike runs; three commits — 2(d), HEIC unit, docs — one push/deploy):**
  ```powershell
  git add routes/utils.py
  git commit -m "feat(health): item 2(d) — SELECT 1 DB liveness in /health; DB down -> 503 {status:degraded}; body stays minimal; monitor check_all still never fails the probe (offline 17/17)"
  ...
  git commit -m "docs(readiness): Section F canonical checklist (Gate 0 HEIC fix=FIX built; load target + 16-18 ceiling burst approved); 2(d) approved-to-ship status"
  git push
  deploy
  # AFTER deploy verified:
  #  1. Render dashboard -> collectioncalc-docker -> Settings -> Health Check Path = /health
  #  2. curl https://collectioncalc-docker.onrender.com/health   -> {"status":"ok","version":"5.6.0"}
  #  3. admin dependency-status (admin JWT) -> runtime.heif = true
  #  4. one normal JPEG grade through the live app (no regression)
  ```
- **NEXT:** (a) Mike runs the block + healthCheckPath + post-deploy checks (+ optionally Render native notifications, now meaningful); (b) real-iPhone HEIC grade = checklist Gate 0 boxes → then F-mobile runs begin; (c) F-load per approved target + F-L4 ceiling burst; (d) item 2(c) Sentry when slotted — the ONLY item-2 remainder once 2(d) is live.
