# Where We Left Off - Jun 27, 2026

## Session 111 (Jun 27, 2026) — State-Recording Protocol enshrined; E3-on-SAM-masks confirmed BUILT; dropped re-capture TOMBSTONED

**MOST RECENT CHANGE: E3 runs on SAM masks of the EXISTING photos, not a controlled-background re-capture, 2026-06-27. This supersedes the re-shoot plan in `E3_CAPTURE_SPEC.md` (now marked dead).**

**Docs-only session so far. Built draft-for-review; Mike commits all (no deploy — docs change only). State-Recording Protocol adopted into the operating model after last night's near-miss.**
...
### NEXT
1. **Mike: keyed gate run** — set `ANTHROPIC_API_KEY`, run the E3 both-sets command (TP+FP in one session), read TP rate + cross-camera FP (must hold 0/6).
2. Mike commits the docs (this entry, `docs/STATE_RECORDING_PROTOCOL.md`, `CLAUDE.md`, `E3_CAPTURE_SPEC.md` tombstone) + the E3 harness (`git add scripts/e3_edge_sequence_test.py`) when ready. No deploy (docs/test only).
3. E3 validates → single-image recovery back on the table for soft launch + learned-seg roadmap justified; fails → single-image genuinely ceilinged (now tested with the demonstrated-working method on clean masks) → multi-view primary.
4. Unchanged launch track: cert-number recovery lookup (lane 1 headline), billing stacking steps 2 & 3, Section F mobile+load, ⏰ 90-day purge ~2026-09-17.
