...
- **Git truth (updated post-ship 2026-07-11 PM):** HEAD = `820b0ae` (Phase 4); `901a49e` (Phase 3) beneath it — Phase 3/4 code fully shipped. Dirty (this arc) = docs (this file + `docs/LAUNCH_READINESS.md`) **+ the 2(f) draft unit** (dependency_monitor.py, routes/utils.py, routes/admin_routes.py, CLAUDE.md, docs/technical/ARCHITECTURE.txt) — two separate commits, don't mix. Pre-existing dirt (NOT this arc, do not bundle): `.claude/worktrees/*` deletions/mods, `scripts/slabguard_crosscamera_test.py`, `tests/SlabGuardTests/TP_RESHOOT_PROTOCOL.md`.
