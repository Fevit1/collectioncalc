...
- [ ] ⏰ **Grade-retention 90-day PURGE (fast-follow — HARD DEADLINE)** — the auto-purge is NOT in the persist pass above. **Deadline: before the first persisted grade submission turns 90 days old** (persist starts the clock the day it deploys — so ~day-90 from go-live, not "someday"). Without it, our LIVE privacy promise ("retained up to 90 days then deleted") becomes false (retain-forever). Mechanism: scheduled job deletes `grade_submissions` rows **and their R2 objects** where `images_purge_after < now()` AND `pinned = FALSE` (reuse `grade_retention.delete_grade_submission`; the `idx_grade_submissions_purge` index + `images_purge_after`/`pinned` columns already exist). Pin rule: when a user files grading feedback, set `pinned = TRUE` on their recent submissions so a complaint's evidence survives. Until this ships, the admin delete tool + `SW_deletion_request_runbook.md` honor erasure manually.
