...
- [ ] **3. Readiness pass — Sections D–F UN-RUN** ⚠️ — A–C done. A–B decisions: gated/batched beta; identification = #1 gate. **C (collection mgmt) run Session ~104** — findings below. Remaining: **D** tier gates, **E** billing end-to-end (the hard gate — ⚠️ Checkout footgun: `create-checkout` writes `stripe_customer_id` immediately; never run from a test account), **F** mobile + load.
- [ ] **4. ID Sigs BROKEN — image fetch/decode failing** ⚠️ LAUNCH GATE (Guard/Dealer paid) — **priority bumped, scope grew.** No longer "cosmetic messageToast": tonight's testing shows "Image decode failed" instantly on multiple comics — dies upstream at the fetch/decode. Leading hypothesis: `fetchImageAsBase64` (`js/utils.js:359`) lacks a `response.ok` check. Full detail + read-only investigation plan in P5 → Bugs to Fix.
...
- [ ] **Identification trustworthiness fix** ⚠️ LAUNCH GATE (private beta) — PLAN OF RECORD decided Session ~104 (June 15), **build next session** (draft-for-review, Mike runs git). One coherent change: (1) GLOBAL Sonnet extraction — flip `comic_extraction.py:483` `'haiku'`→`'sonnet'` tier; (2) honesty gate — grade shows but valuation/verdict HALT on absent/low-confidence issue, confirm-or-fix UI by default, remove `|| '1'` default (`app.html` ~2554), server belt so `/api/sales/valuation` won't blend-all-issues on empty issue (`sales_valuation.py` ~228). Full plan: `docs/technical/IDENTIFICATION_FIX_PLAN_OF_RECORD.md`; analysis: `docs/technical/IDENTIFICATION_HONESTY_REVIEW.md`. Parked: barcode-issue writeback, mylar grade-inflation, model-call-site fallback hardening, GCD/ComicVine catalog (post-launch), pricing-tier review.
