...
**Updated:** June 16, 2026 (Session 105)
...
- [ ] **Extraction resilience (Commit 2) — QUEUED next-session build** (draft-for-review, Mike runs git). `comic_extraction.py` client `timeout=30.0/max_retries=1` + `app.html` `/api/extract` AbortController (75s) + honest "Our identifier is busy right now" copy on timeout/abort (not "Could not identify"). Insurance vs future contention/load; **not urgent** now the auto-fire source is gone. Currently OUT of the working tree.
