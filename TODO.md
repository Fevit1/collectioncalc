...
- [ ] **Grade-submission retention** (spec drafted, build gated on 2 decisions) — unsaved grades retain **nothing** (no photos/grade/subgrades/comic), so grading-accuracy complaints are undiagnosable (surfaced by matbanshee, user 21). Full design in `docs/technical/GRADE_RETENTION_SPEC.md`. New `grade_submissions` table + persist `/api/grade` images to R2 (today they're transient base64, never uploaded). **Mike's 2 decisions before build:** (1) retention approach — recommend **retain all / 90 days / pin-on-feedback**; (2) **privacy/consent** — privacy-policy + ToS disclosure + erasure cascade (users may assume unsaved grades are ephemeral) — settle FIRST. Storage cost negligible (~$0.02–0.20/mo at beta scale). Prerequisite for the calibration thread. NB: the brief's "180° re-read" field is **N/A — no such mechanism exists in grading today** (only barcode/fingerprint rotation).
...
- [ ] **3. Readiness pass — Sections E–F UN-RUN** ⚠️ — A–D done. **D (tier gates) run Session 106** → drove the Tier Honesty Pass (above). Remaining: **E** billing end-to-end (the HARD gate — ⚠️ Checkout footgun: `create-checkout` writes `stripe_customer_id` immediately; **never** run real Checkout/portal from a `test-*` account — webhooks then clobber the tier) — **likely next session's opener**; **F** mobile + load.
