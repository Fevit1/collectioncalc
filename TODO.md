...
**Updated:** June 20, 2026 (Session 108)
...
- [ ] **Section E — MULTI-SUBSCRIPTION STACKING guard** 🔴 LAUNCH BLOCKER (found Session 108) — the portal for +22 showed **THREE concurrent active subs** (Guard $9.99 + Pro $4.99 + a 2nd Pro $4.99). Each checkout created a **NEW** sub instead of modifying the existing one → Pro→Guard "change" stacked, a re-run Pro stacked again. A real subscribe-then-upgrade user could be billed for overlapping plans (~$20/mo). Partly a testing artifact (raw checkout ×3, not an upgrade button). **Investigate read-only first:** does `create-checkout` check for an existing active Stripe sub? Is there a real change-plan flow that MODIFIES the sub (Stripe supports it) or always-create-new? **Fix:** change-plan modifies the existing sub AND/OR checkout refuses if already subscribed. Guard needed before launch regardless.
