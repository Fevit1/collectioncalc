...
**Updated:** June 23, 2026 (Session 109 cont.)
...
- [ ] **GRADING ACCURACY — early signal, worth a real pre-launch look** (captured Session 111, 2026-06-27) — an early user reported grades **off on 3 submitted comics**. Their images were **NOT retained** (pre-retention / unsaved, before the grade-submission capture) → those specific cases **can't be re-run or diagnosed**. **Product framing (not just a CX note):** the slab/no-slab verdict depends on an accurate GRADE **and** accurate COMPS. Today's valuation Fix A/B addressed the **COMPS** half; **grade accuracy is unverified.** Same confidence-gating question as valuation Fix B: **does the grade render confidently when image quality/angle doesn't support it?** — a partial-angle / low-quality grade presented as authoritative is the grading-side twin of the fabricated-FMV confident verdict we just gated. **User contact is GATED on (a) actually improving the grading path AND (b) image retention so we can diagnose a complaint — NOT now** (also needs the support inbox, below). Pairs with the **"Grade-presentation honesty / confidence surfacing"** Polish item (§🟡, app.html — label single-photo/partial-angle grades as lower-confidence) and the **matbanshee** undergrading finding — one theme: honest grade confidence + retained evidence to make accuracy improvable.
...
- [ ] **Signatures v2 — when it ships** (deferred; full design in `docs/technical/SIGNATURES_V2_DESIGN.md`). Grouped: (a) **detection gate** — add an "is a signature visually present?" step before attribution (mirror `routes/signatures.py` Step-1 → abstain at 0); this is the **false-positive correctness fix** (Alex-Ross-on-unsigned-cover) AND a cost saver. ⚠️ NB "abstain on zero pre-filter candidates" is the WRONG lever (pre-filter is era/publisher creator-narrowing, not signature detection). (b) decoupled **user-initiated** delivery (collection-based, not auto on every grade); (c) confidence-verify UX ("we're N% confident — confirm?"); (d) tier-gated **visibility** (don't show the control to Free/Pro); (e) threshold alignment (frontend 0.40 → server floor 0.50, `LOW_CONFIDENCE_THRESHOLD`); (f) multi-signature handling (later) — ⚑ field evidence 2026-09-18: ASM #252 carries a large red signature and a second faint black one; both live runs flagged `multiple_signatures_detected` and analysed only the first. The matcher takes one target and returns one ranking, so a two-signer cover cannot be answered correctly today (WWLO 09-18 night). (g) **never display a below-floor ranking** — two runs on the same cover nine minutes apart reshuffled ranks 2–5 and swapped a name; shipped in the result block 2026-09-18, and it applies to any future surface (report, badge, listing copy).
...
- [x] ~~**Section E billing — core revenue path (pay → correct tier)**~~ ✅ GREEN (Session 108, Jun 20, Stripe TEST mode, throwaway +22 / user 30). Pro checkout → `plan=pro`/`trialing`/both stripe IDs; Guard-as-upgrade → `plan=guard`/`trialing` (**tier-CHANGE works**); cancel-at-period-end scheduled (Jul 4); pre-flight GREEN. **The webhook 500 that blocked all of E was a Render env-var TYPO** — `STRIPE_WEBHOOOK_SECRET` (3 O's) vs the code's `STRIPE_WEBHOOK_SECRET` → "secret not configured" → 500 (value was right, name wrong; renamed + redeployed). All four code theories were wrong. **Webhook hardening shipped & kept** (`routes/billing.py`: `logger.exception` + explicit "secret not configured" msg = what found the typo; real `trialing` status not hardcoded `active`; `_subscription_period_end()` for the `current_period_end`→`items[]` API move; 200-on-handler-error; `requirements.txt` pinned `stripe>=12,<13`). **Two legs still open below: stacking guard (fix) + immediate-cancel→free (test).**
- [x] ~~**Section E — MULTI-SUBSCRIPTION STACKING guard (step 1 of 3)**~~ ✅ SHIPPED & VERIFIED (Session 109, Jun 22) — ~~🔴 LAUNCH BLOCKER~~ **CLOSED.** Investigated read-only: `create-checkout` had **no** existing-sub check (every call minted a new sub — the real bug, not just a testing artifact); there is **no in-code modify/upgrade path** (no `Subscription.modify`; the only in-place change is the Stripe Customer Portal); the webhook is **last-writer-wins** (`handle_subscription_*` match by customer_id only); and account.html "Change Plan" → `/pricing.html` → `create-checkout` routes a tier change straight back into stacking. **Fix shipped** (`routes/billing.py`, additive guard, commit *"fix(billing): stacking guard — refuse create-checkout when a live sub exists"*): before creating a session, if the user has a `stripe_subscription_id` AND status in **active/trialing/past_due**, return **409** `{code:"existing_subscription", manage_via:"customer_portal"}`; checkout allowed only for free→first-paid; canceled/incomplete/unpaid/none can still (re)subscribe; **fails open** on DB error. **Verified in prod:** +22 (already-subscribed) → 409 (was 200+checkout_url); Stripe still shows exactly 3 subs (refused before any Stripe call). Steps 2 & 3 below are hardening on the now-closed blocker.
- [ ] **Section E — STACKING step 2: UI redirect** (hardening, after step 1) — point account.html **"Change Plan"** at `openPortal()` instead of `/pricing.html`, and have pricing.html/account.html **detect the 409 `code:"existing_subscription"`** and auto-open the customer portal instead of alerting the error string (a smooth redirect for already-subscribed users). Also verify in the Stripe dashboard that the Customer Portal's **plan-switching ("switch plans") is enabled** for the Pro/Guard products (dashboard config — no code); flag if it isn't.
- [ ] **Section E — STACKING step 3: webhook hardening** (the scary latent bug, after step 1) — `handle_subscription_deleted` (`routes/billing.py:778`) matches by `customer_id` only and reverts to free on **ANY** sub deletion → a stacked user who cancels ONE sub is dropped to free while Stripe keeps billing the others. Change it to revert only if the deleted `sub.id` matches the user's stored `stripe_subscription_id` (or re-resolve their remaining active sub). Optional follow-on: `handle_subscription_updated` ignores events for a sub that isn't the user's-of-record (kills last-writer-wins tier flicker). **Do alongside the immediate-cancel→free test below — same handler.**
- [ ] **Section E — test immediate cancel → `plan=free`** (the `customer.subscription.deleted` downgrade/teardown webhook) — the last untested E leg. The portal only did cancel-at-period-end (Jul 4 schedule), so nothing has terminated yet; needs a "cancel immediately" to fire the downgrade webhook and confirm the revert to free.
- [ ] **Signup "too many requests" — launch mitigation (Resend)** — read-only finding confirmed (Session 108): NOT our app and NOT Cloudflare (no signup rate-limit in code — `flask-limiter` not even a dep; API posts straight to Render). Almost certainly **Resend's free-tier daily email cap (~100/day)** — accounts still create (failure swallowed → `email_send_failed`), but verification **emails** break under load. **Mitigation before Aug 21:** Resend paid plan + verified sending domain. Also: NO abuse rate-limit on signup at all → consider a gentle per-IP limit post-launch.
- [x] ~~**Lookup-demand instrumentation (coverage moat)**~~ ✅ SHIPPED & VERIFIED (Session 108, commit `daf9050`) — logs one row per valuation lookup (`/api/sales/valuation` + `/api/sales/fmv`) so ~28K lookups/mo become a demand-ranked backfill list. `migrations/add_lookup_demand.sql` + `lookup_demand.py` (fire-and-forget daemon thread, never blocks/raises) + 3 hooks in `routes/sales_valuation.py`. Captures comp counts, fmv_method, estimated/no_data, user_id (distinct-user ranking), is_internal (admin pre-filter). Verified: row written for an ASM #300 lookup. **Now collecting; do not over-read sparse early-beta data.** Read-only "top thin-data titles" query is in the brief, runnable from a `coverage_assessment.py`-style script. **Origin = read-only coverage assessment** (eBay 53,840 rows / Whatnot 9,677; fresh but WIDE-and-SHALLOW — 89% of keys have 1–2 comps, grade-specific FMV solid on ~268 books; ~27% of rows over-filtered by variant/lot/reprint). **Follow-on work this enables:** (a) demand-driven DEPTH backfill, (b) **barcode-variant-subtyping** to reclaim ~11K filtered variant comps, (c) steady capture cadence (capture is manual/bursty — Apr–May stalled).
- [ ] **3. Readiness pass — Section F UN-RUN** ⚠️ — A–E done (**E core path GREEN Session 108**, two legs tracked above). Remaining: **F** mobile + load. (⚠️ Checkout footgun still applies: `create-checkout` writes `stripe_customer_id` immediately; **never** run real Checkout/portal from a `test-*` account — webhooks then clobber the tier. Use a fresh throwaway.)
...
### Slab Guard — recovery capability (Session 109 cont., Jun 23)
- [x] ~~**Slab Guard Vision arbiter → Opus 4.8 + fallback + cost fix**~~ ✅ SHIPPED & DEPLOYED (Session 109 cont., commit `647bca2`, Render Events green) — `compare_covers_with_vision` now defaults to **Opus 4.8 via `call_with_fallback('opus')`** (was a frozen direct `client.messages.create(model=SONNET)` with NO fallback — the whole cross-camera copy verdict rides on this one call). `models.py` opus head 4-6→4-8 (4-7/4-6 fallbacks); model-aware cost formula ($5/$25 Opus, $3/$15 Sonnet — correct for both A/B arms); `arbiter_model` field added. Explicit `model=` override still honored (harness A/B). **Cost ~1.67× (not the brief's 5×), on a call that barely fires** — the shipped extension runs quant-only; the arbiter only fires on manual `/api/monitor/compare-copies` + the harness. Functional `arbiter_model=claude-opus-4-8` check deferred to the harness run.
- [ ] **Cross-camera RECOVERY test — Mike's photo shoot + harness A/B** 👤⚠️ — `scripts/slabguard_crosscamera_test.py` (read-only, not wired to prod) + capture protocol READY. Mike: source a **matte untextured contrasting background** (texture = #1 false-positive cause), confirm ANTHROPIC_API_KEY + opencv in venv, shoot ~20 photos (5 issues × 2 copies × 2 phones, front cover, `issue<N>_copy<A|B>_phone<1|2>.jpg`), run the harness twice (no `--model` = Opus 4.8 default; `--model claude-sonnet-4-6` for the A/B). **DECISIVE METRIC = cross-camera false-positive rate (different copy same issue must NOT match); want 0** — gates whether "recovery" can go on any booth copy / pricing tier. Reconciliation: prior testing was real but mostly **cross-IMAGE / same-camera** (only n≈1 same-copy **cross-camera**); this run produces the missing cross-camera RATES.
- [ ] **Cert-number recovery lookup (small build) — the honest marketable HEADLINE** — the CGC/CBCS cert is already **OCR'd** (`comic_extraction.py`), **stored + indexed** (`comic_registry`/`collections`), and **displayed** in verify lookup, but **never matched on** (`find_matches()` keys only on hashes; no cert→registered-copy endpoint). Wiring a cert lookup is the **lowest-effort, highest-reliability** slabbed-recovery vector (no CV research). **Positioning decided:** slabbed→cert = marketable recovery headline; raw/photo-matching stays provenance+monitoring framing until the cross-camera FP-rate proves it.
...
- [ ] **5. Resilience gap (model audit, June 15)** — ~7 of 12 model call sites still pass static constants with NO `call_with_fallback` (Chrome vision, signature v1/v2, eBay gen, admin); they'd break with no auto-fallback if a head model string retires. **Slab Guard CV arbiter now hardened** (Session 109 cont. — routed through `call_with_fallback('opus')` with the Opus 4.8 switch). Harden the rest later. Not urgent.
...
- [ ] **"Photo too small" error doesn't say WHICH photo** — name the offending photo (front/back/spine/centerfold). The validator already KNOWS which image/position failed; it's just not surfaced in the message, so the user re-shoots all images by guesswork. Small fix (put the failing image's identity/position in the error string). **Re-confirmed live during Fix B testing (Session 111, 2026-06-27).** ⚠️ Higher value at GalaxyCon — re-shooting all images because you don't know which failed = booth-line friction. Anchor: `app.html` multi-image upload validation + grade-result too-small handling (~app.html:2415–2423).
- [ ] **Duplicate "grade a comic" link** on the landing page — de-dupe.
- [ ] **eBay capture extension: multi-page banner looks clickable but does nothing** (~5 min, someday — NOT now) — the "Page 1 of 2 — click Next →" label (`content.js:850–852` + `.sw-page-info` style `:604–611`) is a bordered amber chip with a `→` arrow, so it reads as a button but has no handler; it's a passive prompt to click eBay's own Next. Honest-label + de-style fix (drop the arrow, reword to "click eBay's Next to capture page 2", `cursor:default`) — relevant only if someone else ever runs capture or for fast booth use. **Mike now manually pages multi-page searches**, so this is cosmetic dedup against the misleading-UI trap, **not** the data-completeness fix (that's recorded in LAUNCH_READINESS). ⚠️ **NEVER** auto-paginate (ban risk — [[feedback_ebay_capture_safety]]); page-tracking assist skipped (redundant with manual paging).
